...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("SELECT e FROM Event e LEFT JOIN FETCH e.category c " +
           "WHERE e.status = ru.savvy.soldo.model.enums.EventStatus.PUBLISHED " +
           "AND e.season IS NULL " +
           "AND c.season = :season " +
           "AND c.format NOT IN (ru.savvy.soldo.model.enums.EventFormat.SESSION_OUTDOOR, " +
           "                     ru.savvy.soldo.model.enums.EventFormat.SESSION_CITY) " +
           "AND e.startDate IS NOT NULL AND e.startDate >= :date")
    List<Event> findUpcomingBySeason(@Param("season") ru.savvy.soldo.model.enums.SeasonType season,
    ...
    @Query("SELECT e FROM Event e LEFT JOIN FETCH e.category c " +
           "WHERE e.status = ru.savvy.soldo.model.enums.EventStatus.PUBLISHED " +
           "AND e.season IS NULL " +
           "AND (c.season = ru.savvy.soldo.model.enums.SeasonType.ALL_YEAR OR e.category IS NULL) " +
           "AND (c.format IS NULL OR c.format NOT IN (ru.savvy.soldo.model.enums.EventFormat.SESSION_OUTDOOR, " +
           "                                          ru.savvy.soldo.model.enums.EventFormat.SESSION_CITY)) " +
           "AND e.startDate IS NOT NULL AND e.startDate >= :date")
    ...
    @Query("SELECT e FROM Event e LEFT JOIN FETCH e.category " +
           ...
           "AND e.status = ru.savvy.soldo.model.enums.EventStatus.PUBLISHED " +
